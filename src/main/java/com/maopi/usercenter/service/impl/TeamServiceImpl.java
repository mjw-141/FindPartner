...
import com.maopi.usercenter.model.domain.UserTeam;
import com.maopi.usercenter.model.dto.TeamQuery;
import com.maopi.usercenter.model.enums.TeamStatusEnum;
...
import com.maopi.usercenter.service.UserTeamService;
...
//            TeamStatusEnum statusEnum = TeamStatusEnum.getEnumByValue(0);
//            if (statusEnum == null) {
...
//            }
            // 根据状态来查询
            Integer status = teamQuery.getStatus();
            TeamStatusEnum statusEnum = TeamStatusEnum.PUBLIC; // 默认为PUBLIC

            if(status==null){

            }else{
                TeamStatusEnum tempStatusEnum = TeamStatusEnum.getEnumByValue(status);
                if (tempStatusEnum != null) {
    ...
    public boolean updateTeam(TeamUpdateRequest teamUpdateRequest, User loginUser) {
        ...
        if (statusEnum.equals(TeamStatusEnum.SECRET)) {
            if (StringUtils.isBlank(teamUpdateRequest.getPassword())) { // 修改这里，检查密码是否为空
    ...
    public boolean joinTeam(TeamJoinRequest teamJoinRequest, User loginUser) {
        ...
        //只有一个线程能获取到锁
        RLock lock=redissionClient.getLock("maopi:join_team");
        ...
        try {//等待时间，没抢到就不等待；锁过期时间；尝试获取锁的次数
            ...
            while (true){
                if (lock.tryLock(0, 30000L, TimeUnit.MILLISECONDS)) {
                    ...
                    userTeamQueryWrapper.eq("userId", userId);
                    userTeamQueryWrapper.eq("teamId", teamId); // 注意这里应该是 teamId 而不是 userId
